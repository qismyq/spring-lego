...
import com.springlego.autoconfigure.user.dto.vo.post.PostSaveReqVO;
import com.springlego.autoconfigure.user.errorhandler.UserErrorCodeEnum;
import com.springlego.autoconfigure.user.mapper.PostMapper;
import com.springlego.autoconfigure.user.service.IPostService;
...
public class PostServiceImpl implements IPostService {
    ...
    private void validatePostNameUnique(Long id, String name) {
        ...
        if (id == null) {
            throw new ErrorMessageException(UserErrorCodeEnum.MULTIPLE_POST_NAME);
        }
    ...
    private void validatePostCodeUnique(Long id, String code) {
        ...
        // 如果 id 为空，说明不用比较是否为相同 id 的岗位
        if (id == null) {
            throw new ErrorMessageException(UserErrorCodeEnum.MULTIPLE_POST_CODE);
        }
        if (!post.getId().equals(id)) {
            throw new ErrorMessageException(UserErrorCodeEnum.MULTIPLE_POST_CODE);
        }
    ...
    private void validatePostExists(Long id) {
        ...
        if (postMapper.selectById(id) == null) {
            throw new ErrorMessageException(UserErrorCodeEnum.POST_NOT_FOUND);
        }
    }
    ...
    public void validatePostList(Collection<Long> ids) {
        ...
        ids.forEach(id -> {
            ...
            if (!CommonStatusEnum.ENABLE.getStatus().equals(post.getStatus())) {
                throw new ErrorMessageException(UserErrorCodeEnum.POST_NOT_ENABLE);
            }
        });
    }
}
